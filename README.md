...
リリースページから `Racker Terminal_<version>_x64-setup.exe` (NSIS) または
`Racker Terminal_<version>_x64_ja-JP.msi` (MSI) をダウンロード。
...
v1.0 では未署名。Windows Defender SmartScreen の警告が出る可能性あり。
警告画面で「詳細情報」→「実行」で起動可能。

### アップデート

v1.x へのアップデートは、Releases ページから新しいインストーラーをダウンロードして上書きインストールしてください (自動更新は将来対応予定)。
...
| **配布 (P4-D)** | msi/exe インストーラー生成。MonaspiceNe NF (woff2) をバンドル — フォント未インストール環境でも正しく描画 |
