...
export interface AppState {
  ...
  duplicateTab: (tabId: string) => string;  // 同一グループ内に shell/cwd/env で新規 spawn
...
**restart フロー**:
1. 「Click to restart」クリック → `setTabStatus(tabId, 'spawning')`
2. TerminalPane が spawning に遷移 → useEffect で `spawnPty` 再実行
3. 成功: `setTabStatus(tabId, 'live', newPtyId)`
4. 失敗: `setTabStatus(tabId, 'crashed')` + エラーオーバーレイ再表示
...
| [api] [web] ...   | TerminalPane(tabB) [invisible]           |
|                   | TerminalPane(tabC) [invisible]           |
| v  work           |                                          |
|    [o] api        | active tab の xterm が前面に表示         |
|    [o] web        | crashed tab はオーバーレイを重ねて表示   |
| >  dev            |                                          |
|    [!] old-db     | （全タブ空の場合はプレースホルダー）      |
|                   |                                          |
| [+ New Group]     |                                          |
+-------------------+------------------------------------------+
```
...
### Unit H: StrictMode＋HMR 確認

- 対象ファイル:
  - `src/main.tsx` — StrictMode 有効化確認
  - `vite.config.ts` — HMR 設定確認
- 変更内容:
  - StrictMode を有効にした状態で terminalRegistry の二重 dispose が起きないことを手動確認
  - HMR（モジュール差し替え）後もターミナルが継続動作することを手動確認
  - **WebGL は対象外**（Phase 3 スコープ）
  - 問題があれば releaseRuntime の queueMicrotask タイミングを調整
