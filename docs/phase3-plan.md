...
**Phase 4 開始 (2026-04-28〜)**: Unit P4-G から実装中。

1. **Unit P4-G 実装中**: お気に入り改善 (OSC 7 cwd 追跡 + 手動登録 UI + 編集機能)
   - 詳細は `docs/phase4-plan.md` を参照
2. **次の優先候補**:
   - 「日常使い」のために **永続化** (2.1, 2.2, 2.20) が最有力 (Unit P4-A)
   - 「自分の PC で常用」のために **配布** (2.17, 2.15) を組み合わせると v1.0 になる (Unit P4-D)
3. **Phase 4 スコープ**: `docs/phase4-plan.md` §7 後続フェーズ送り項目を参照。
