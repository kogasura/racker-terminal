...
P-D3 で `titleSub.dispose()` と `compositionAbort.abort()` が追加、Phase 3 Unit P-C1 で `webglAddon?.dispose()` が追加、
Phase 4 P-G で `oscSub.dispose()` が追加されたため、
Unit H の StrictMode 検証でもこの順序（isDisposed → setOnEvent(null) → onDataSub → titleSub → oscSub → compositionAbort → webglAddon → fitAddon → ptyHandle → term）が
